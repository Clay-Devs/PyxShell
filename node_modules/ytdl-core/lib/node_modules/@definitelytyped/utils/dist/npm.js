...
exports.NpmPublishClient = exports.UncachedNpmInfoClient = exports.withNpmCache = exports.Registry = exports.npmApi = exports.githubRegistry = exports.npmRegistry = exports.githubRegistryHostName = exports.npmRegistryHostName = void 0;
require("./types/npm-registry-client");
const assert = require("assert");
const fs_extra_1 = require("fs-extra");
const npm_registry_client_1 = __importDefault(require("npm-registry-client"));
...
async function withNpmCache(uncachedClient, cb, cacheDir = defaultCacheDir) {
    ...
    if (cacheFileExists) {
        ...
        const cachedJson = (await fs_extra_1.readJson(cacheFile));
        log.info(`Cache file ${cacheFile} exists, copying to map...`);
        unroll = collections_1.recordToMap(cachedJson, npmInfoFromJson);
    }
    else {
    ...
    await fs_extra_1.writeJson(cacheFile, collections_1.mapToRecord(unroll, jsonFromNpmInfo));
    return res;
...
class UncachedNpmInfoClient {
    ...
    async fetchNpmInfo(escapedPackageName) {
        ...
        await miscellany_1.sleep(0.01); // If we don't do this, npm resets the connection?
...
class NpmPublishClient {
    ...
    static async create(token, config, registry = Registry.NPM) {
        switch (registry) {
            case Registry.NPM:
                return new NpmPublishClient(new npm_registry_client_1.default(config), { token }, exports.npmRegistry);
            case Registry.Github:
                return new NpmPublishClient(new npm_registry_client_1.default(config), { token }, exports.githubRegistry);
            default:
                assertions_1.assertNever(registry);
        }
    }
    async publish(publishedDirectory, packageJson, dry, log) {
        const readme = await fs_extra_1.readFile(fs_1.joinPaths(publishedDirectory, "README.md"));
        return new Promise((resolve, reject) => {
            const body = io_1.createTgz(publishedDirectory, reject);
    ...
    deprecate(packageName, version, message) {
        const url = url_1.resolve(exports.npmRegistry, packageName.replace("/", "%2f"));
        const params = {
            message,
            version,
            auth: this.auth
...
function npmInfoFromJson(n) {
    return {
        ...n,
        distTags: collections_1.recordToMap(n["dist-tags"], miscellany_1.identity),
        // Callback ensures we remove any other properties
        versions: collections_1.recordToMap(n.versions, ({ typesPublisherContentHash, deprecated }) => ({
            typesPublisherContentHash,
            deprecated
        })),
        time: collections_1.recordToMap(n.time)
    };
}
function jsonFromNpmInfo(n) {
    return {
        ...n,
        "dist-tags": collections_1.mapToRecord(n.distTags),
        versions: collections_1.mapToRecord(n.versions),
        time: collections_1.mapToRecord(n.time)
    };
}
